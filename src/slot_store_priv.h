 ...
 *
 * slot_store private layout (redesign §2.1).
 *
 ...
 * deliberately white-box: §4 specifies they assert slots[] / pending_delete /
 * slot_generation outcomes directly, which is the whole point of pinning the
 * dual-write ordering. No other module includes this header.
 *
 ...
 * the firmware needs lives in the nvs adapter, not in this ordering logic (§3).
 */
...
struct slot_store {
    ...
    struct dm_slot draft; /* the recording buffer (§2.7.4) */
