...
>   **Open (test-infra, not a rewrite defect):** the two write-heavy NVS-roundtrip parity cases were *removed* from the corpus (32, not 34) because their flash-persisting twins cross-contaminate the shared `flash.bin`/settings partition with the legacy NVS tests — the §4.3 hazard. With them gone, the legacy `core/nvs_roundtrip` + `core/multi_slot_nvs` still fail on stale-flash boot state (16-event leftover slots), independent of the new stack (the new stack's NVS save/reload ran green while its parity cases were present; `core__nvs_load` passes). This is harness flash isolation, to resolve before the step-8 cut-over.
> - [ ] **Step 8** — single cut-over (preconditioned on resolving the legacy NVS flash-isolation flake, then the full corpus green). [ ] **Step 9** — footprint pass.
