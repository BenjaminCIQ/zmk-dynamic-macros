...
Status: **in progress** · updated 2026-06-12 · branch `docs/architecture-redesign`
(steps 0–8 landed; the cut-over removed the legacy path and the parity scaffolding,
leaving the modular stack as the sole path — only step 9, the footprint pass, remains)
...
> **`DM_DELETE_DEFERRED` / `DM_DELETE_STALE` were added at the pre-cut-over review
> (2026-06-10)** to make the delete timing explicit instead of out-of-band. A RAM
> delete completes synchronously (`slot_store_delete` returns `DM_OK`, the machine
> speaks DELETED now); an NVS delete is enqueued (`DM_DELETE_DEFERRED`, the machine
> stays silent and the DELETED notification + speech wait for `deliver_async` on
> completion). Without the distinct code the machine could not tell the two apart
> from the return value and spoke DELETED *both* times. Likewise `slot_store_complete_delete`
> returns `DM_DELETE_STALE` (not `DM_OK`) when the completion is a no-op (slot not
> pending or generation stale), so `deliver_async` drops it rather than speaking a
> phantom DELETED for an op the slot already moved past.
...
The `native_sim` suite stays green at every step (see §5). The NVS-test flakiness was
**shared-`flash.bin` test-harness contention, not a firmware bug** — and it is now fixed
at the root (2026-06-10). Cause: Zephyr's flash simulator defaults to a *file* backing
(`./flash.bin`, relative to the process cwd, mmap'd `MAP_SHARED`, erased only when the
file is empty), and ZMK's `run-test.sh` runs every test binary argument-less from one
shared cwd, up to four in parallel — so every flash-enabled test read and wrote the same
`flash.bin`, leaking a previous test's slots into the next boot (and corrupting
concurrent runs). Fix: the module `Kconfig` defaults the upstream
`NATIVE_EXTRA_CMDLINE_ARGS` symbol (Zephyr ≥ 4.0, present in the pinned 4.1) to
`"--flash_in_ram"` whenever `..._TEST_RELOAD && FLASH_SIMULATOR`, which the flash
simulator treats exactly like a shell-passed flag: flash lives in process-private RAM,
pristine at every boot. No harness changes, no cleanup responsibilities in the tests
themselves, and the semantics under test are intact because `DM_TEST_RELOAD` exercises
the full settings/NVS round-trip *within* one process. Any future persistence test
inherits the isolation just by setting `..._TEST_RELOAD=y` in its conf.
...
> - [x] **Step 7** — the full Zephyr-coupled stack landed beside the live path (parallel stack, reachable only from the parity harness): the new `behavior_dynamic_macro_v2` shell + 5b feedback pump + 6b `dm_events` (notify-code→widget-event map + raise, `dm_get_*` over `slot_store`/`dm_query`) + `dm_nvs` wiring (sink adapter, system-queue completion running `slot_store_complete_delete` + `dm_machine_deliver_async`, boot restore, knob persist, export read-back, `DM_TEST_RELOAD`). The §5.1 shell-inventory checklist is met. **Machine additions** (host-tested, 95/95): `dm_machine_timeout` and `dm_machine_play_finished` up-calls, and an `arm_timeout`/`cancel_timeout` callback pair replacing the internal `timeout_pending` flag so the shell timer tracks the pending state even when re-armed from inside the pump. **Build seam:** `CONFIG_…_NEW_STACK` selects the new stack in `CMakeLists.txt` (default off — the shipping path is byte-for-byte untouched); `DM_NEW_STACK` guards the new query/shell symbols; `dm_config.h` sources Kconfig sizing under `__ZEPHYR__` and a shared `DM_SLOT_DEFINED` guard lets new-stack TUs include both slot headers, so `struct dm_slot` is identical across every TU. **Verification:** host suite green for the pure additions (95/95, incl. the new machine up-calls); the Zephyr code is verified by the §5.2 e2e parity harness on the CI keymap-snapshot rail — *not* host-buildable, so its first real check is CI. **Result: the new stack matches the legacy oracle on the full parity corpus — 32/32 cases green** (`tests/parity/e2e/`, generated by `generate.sh` from the legacy snapshots). Getting there flushed out and fixed several real defects against the live oracle: a state-machine clobber (cues must route through `TYPING_FEEDBACK` with the destination parked as the return-state, exactly as the old `start_feedback`/`feedback_complete` did — not direct state writes, which the OFF-path `typing_finished` wiped to a stale return-state), the `dm_events` log format (missing the `state=` field), and notify ordering for RECORDING-context events (raise before the state write so the coarse state matches).
>   ~~**Open (test-infra, not a rewrite defect):** the two write-heavy NVS-roundtrip parity cases were *removed* from the corpus (32, not 34) because their flash-persisting twins cross-contaminate the shared `flash.bin`/settings partition with the legacy NVS tests — the §4.3 hazard.~~ **Resolved 2026-06-10:** root cause was the flash simulator's shared file backing (full diagnosis + fix in §4.3 — `NATIVE_EXTRA_CMDLINE_ARGS="--flash_in_ram"` Kconfig default under `..._TEST_RELOAD`). Every test process now boots a private, pristine flash, so the legacy `core/nvs_roundtrip` + `core/multi_slot_nvs` stale-boot failures (16-event leftover slots) are gone at the source, and the two dropped parity cases are restored — **corpus back to 34** (regenerated via `generate.sh`, byte-identical to the pre-drop versions). The first CI run with the fix (run 27270199161) went 67/69 and exposed a **poisoned oracle**: `core/multi_slot_nvs`'s committed snapshot contained an extra B press/release pair (slot 1 playing "B B") that was stale-flash pollution baked in at recording time — the clean run produces exactly the keymap-derived sequence (live A, B, A+B; reload; play A, B, A+B). Snapshot corrected (extra pair removed) and the parity copy regenerated; both legacy NVS cases plus their parity twins otherwise green.
> - [x] **Post-step-7 deepening (2026-06-10)** — the machine→feedback seam was collapsed. The `dm_machine_callbacks` vtable carried **24 `speak_*` slots** (one per message kind), the shell wired 24 trivial `sp_*` adapters that each rebuilt a `dm_feedback_spec`, and the machine re-named a `speak_X` per transition — re-expanding at the seam the very enum `dm_feedback_spec` already collapses (the 23-clone shape ADR-0001 deletes, relocated to the callback surface). Now **one** `speak(ctx, const dm_feedback_spec*)` slot: the machine builds the spec inline and calls it once per transition; transitions that type nothing (preview) call `dm_machine_typing_finished` directly (no phantom speak); the vestigial `speak_erase` is gone (the pump arms its own backspaces). The spec type (`dm_fb_kind`/`dm_feedback_spec`) moved to a new pure **`dm_spec.h`** so the pure machine and `dm_feedback_build` share it without depending on each other. **The knob legality leak (review candidate 2) folded in**: `do_knob` now drives the effect through a new `apply_knob(ctx, dm_command)` callback *inside* the transition, so the shell stops re-running the command and reading `state==TYPING_FEEDBACK` to reconstruct the ALLOWED/IGNORED verdict — knob dispatch is now uniform with REC/DEL/MOV. Host suite 97/97 (the `test_machine.c` fake reworked to one `fake_speak` mapping kind→tag, +2 knob tests; `erase_due` test now asserts park-only). The Zephyr shell/pump edits ride the §5.2 e2e parity rail (not host-buildable).
> - [x] **Post-step-7 deepening, part 2 (2026-06-10)** — the internal notification code list (`DM_EVT_*`) was de-duplicated. It had three hand-synced definitions: 14 `#define`s in `dm_machine.c` (the producer), a 14-value `enum` in `dm_events.c` ("mirror the DM_EVT_*"), and a lone `#define DM_EVT_PLAY_FINISHED 6` in the shell, kept identical by comment. Now **one** pure **`dm_notify.h`** owns `enum dm_notify_code` (+ `DM_EVT__COUNT` sentinel); the machine, `dm_events`, and the shell all include it. The `dm_events::map_event` switch — genuine translation to the public `zmk_dynamic_macro_event_type`, whose ordering deliberately diverges from the internal one — **stays**, and lost its `default` so a future code that misses a case is flagged by `-Wswitch`. The `notify` vtable slot stays `int` (no ABI ripple into the shell). Host suite 97/97 (the machine sources its codes from the shared header; the `dm_events`/shell consumer edits ride the §5.2 e2e parity rail). Mirrors the `dm_spec.h` precedent: one pure header per shared value type.
> - [x] **Pre-cut-over parity review (2026-06-10)** — a showcase-criteria review found four new-stack parity defects, all hidden in corpus blind spots (every NVS snapshot case runs `FEEDBACK_OFF` without `EVENTS`, so NVS×feedback and NVS×notification interactions were untested). All four fixed test-first; host suite **103/103** (+6). (1) **NVS delete spoke/raised DELETED twice** — the machine spoke immediately *and* on completion, a failed NVS delete showed DELETED-then-FAILED, and a stale completion spoke a phantom DELETED. Fix: `slot_store_delete` returns a new `DM_DELETE_DEFERRED` for the enqueued NVS path (the machine stays silent and lets `deliver_async` speak), `slot_store_complete_delete` returns a new `DM_DELETE_STALE` for a superseded op (`deliver_async` drops it), and `deliver_async` now parks/enters TYPING_FEEDBACK before speaking so the machine reads busy during the deferred typing — as the old deferred `dm_feedback_deleted` did. (2) **Completed auto-erase never cleared its flags** — a clean backspace drain left `erase_active`/`erase_in_progress` set and restored the *wrong* return-state (the last speak's, not the parked erase state), so a later keycode fired a phantom `erase_cancel` resurrecting a dead state; multi-batch erase was also never continued. Fix: new `dm_machine_erase_finished` up-call (restores the parked state + clears the flag, symmetric with `erase_cancel`), and the pump's drain path routes an erase completion through it, rescheduling a remaining batch. (3) **REC from PREVIEW_PENDING was IGNORED** — the old `cmd_record` blocklist omits PREVIEW_PENDING, so the matrix cell flips to ALLOWED and `do_rec` cancels the pending timeout. (4) **Deferred assign-persist queue-full was silently dropped** — `typing_finished` discarded `store_persist`'s result; it now speaks SAVE QUEUE FULL + raises ERROR_QUEUE_FULL from the settled state, as the old `feedback_complete → dm_save_slot` did. New `dm_result` codes (`DM_DELETE_DEFERRED`, `DM_DELETE_STALE`) documented in §2.0. The Zephyr pump/nvs edits ride the §5.2 e2e parity rail; new native_sim cases for NVS-delete-at-VERBOSE+EVENTS and erase-complete-then-type are a recommended follow-up.
> - [x] **Step 8** — single cut-over (2026-06-12). The legacy path
>   (`behavior_dynamic_macro.c` / `dm_feedback.c` / `dm_storage.c` + `dm_feedback.h`
>   / `dm_internal.h`) and the entire parity scaffolding (`tests/parity/`) were
>   removed wholesale; the `NEW_STACK` Kconfig + `DM_NEW_STACK` compile-def seam was
>   stripped so the modular stack is the sole, unconditional path. The 37-case
>   `native_sim` snapshot corpus — unchanged since before the rewrite — now runs
>   against the new path directly and is the permanent net (the e2e parity twins,
>   having served as the new-vs-old oracle, are retired). A separate follow-up
>   commit drops the `v2`/new/old naming. [ ] **Step 9** — footprint pass.
